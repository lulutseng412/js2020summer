...
export default {
  ...
  methods: {
    ...
    getCoupons () {
      this.isLoading = true
      const url = `${this.api.path}/api/${this.api.uuid}/admin/ec/coupons`
    ...
    updateCoupon () {
      this.isLoading = true
      let api = `${this.api.path}/api/${this.api.uuid}/admin/ec/coupon`
      let httpMethod = ''
      let status = '新增成功囉'
      if (this.status === 'created') {
        httpMethod = 'post'
      } else {
        api = `${this.api.path}/api/${this.api.uuid}/admin/ec/coupon/${this.tempCoupon.id}`
        status = '更新成功囉'
        httpMethod = 'patch'
      }
      // 針對日期做組合重新寫入到物件中
      ...
      this.$http[httpMethod](api, this.tempCoupon).then(() => {
        ...
        this.$bus.$emit('message:push', '出現錯誤惹，好糗Σ( ° △ °|||)︴', 'danger')
        this.isLoading = false
      })
    },
    delCoupon () {
      this.isLoading = true
      const api = `${this.api.path}/api/${this.api.uuid}/admin/ec/coupon/${this.tempCoupon.id}`
      this.$http.delete(api).then(() => {
        $('#delCouponModal').modal('hide')
        this.$bus.$emit('message:push', '刪除成功囉', 'success')
        this.getCoupons()
        this.isLoading = false
      }).catch(() => {
        this.$bus.$emit('message:push', '出現錯誤惹', 'danger')
        this.isLoading = false
      })
    }
  }
